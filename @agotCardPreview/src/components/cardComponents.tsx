...
import {
    ...
    PlotStat as PlotStatType,
    Type as TypeType,
    Watermark as WatermarkType,
    IRenderCard
...
type CardWrapperProps = BaseElementProps & {
...
export const Card = memo(
    ({
        children,
        card,
        orientation = defaultOrientation(card.type),
        rounded = true,
        scale = 1,
        className,
        classNames: classGroups,
...
type CardProps = BaseElementProps & {
    classNames?: { wrapper?: string; inner?: string };
    styles?: { wrapper?: CSSProperties; inner?: CSSProperties };
    scale?: number;
...
export const Type = memo(({ children: type, className, style }: TypeProps) => {
    return (
        <div
            className={className}
            style={{ position: "relative", top: px(-5), left: px(2), fontSize: px(8), ...style }}
        >
            {type}
        </div>
    );
...
export const Ability = memo(({ children: text, className, style }: AbilityProps) => {
    const convertToHtml = (htmlString: string): React.ReactNode[] => {
        const raw = htmlString
            ...
            .replace(
                /<p>((?:\s*[+-]\d+ (?:Income|Initiative|Claim|Reserve)\.?\s*)+)<\/p>/gi,
                "<plotModifiers>$1</plotModifiers>"
            )
            // ...and wrap each plot modifier in a span within that class
            .replace(
                /\s*([+-])(\d+) (Income|Initiative|Claim|Reserve)\.?\s*/gi,
                (_: string, modifier: string, value: string, plotStat: string) =>
                    `<plotModifier name="${plotStat.toLowerCase()}" modifier="${modifier}">${value}</plotModifier>`
            )
        ...
        const transformNode = (node: ChildNode | Element, key: number) => {
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                switch (el.tagName.toLowerCase()) {
                    ...
                    case "b":
                        return (
                            <b key={key} style={{ fontWeight: "700" }}>
                                {children}
                            </b>
                        );
                    case "i":
                        return (
                            <i key={key} style={{ fontStyle: "italic", fontWeight: "700" }}>
                                {children}
                            </i>
                        );
                    case "ul":
                        return (
                            <ul
                                ...
                                style={{
                                    listStyleType: "disc",
                                    marginBlockStart: em(0.25),
                                    marginBlockEnd: em(0.25),
                                    paddingInlineStart: em(2)
                                }}
                            >
                                {children}
                            </ul>
                    ...
                    case "li":
                        return (
                            <li key={key} style={{ paddingTop: em(0.1), paddingBottom: em(0.1) }}>
                                {children}
                            </li>
                        );
                    case "icon":
                    ...
                    case "plotmodifier":
                        return (
                            <PlotModifier
                                key={key}
                                type={el.getAttribute("name") as PlotStatType}
                                modifier={el.getAttribute("modifier") as "+" | "-"}
                                inline={true}
                            >
                                {parseInt(children[0] as string) as number}
                            </PlotModifier>
                        );
                    default:
                        return <span key={key}>{children}</span>;
    ...
    return (
        <div
            className={className}
            style={{ display: "flex", flexDirection: "column", flexGrow: 1, gap: em(0.25), ...style }}
        >
...
export const Designer = memo(({ children: designer, className, style }: DesignerProps) => {
    return (
        designer && (
            <div
                className={className}
                style={{ flexGrow: 1, fontWeight: "700", fontSize: px(11), paddingTop: em(0.5), ...style }}
            >
                {designer}
            </div>
        )
...
export const PlotModifier = memo(({ className, style, type, modifier, children: value }: PlotModifierProps) => {
    ...
    return (
        value && (
            <PlotStat className={className} style={style} type={type}>
                {actualValue}
            </PlotStat>
        )
    );
});
type PlotModifierProps = Omit<BaseElementProps, "children"> & {
    type: PlotStatType;
    modifier: "+" | "-";
    children?: PlotValueType;
    inline?: boolean;
};
...
type PlotStatProps = Omit<BaseElementProps, "children"> & {
    type: PlotStatType;
    children?: PlotValueType | `${"+" | "-"}${PlotValueType}`;
};
...
type DeckLimitProps = Omit<BaseElementProps, "children"> & {
    type?: TypeType;
