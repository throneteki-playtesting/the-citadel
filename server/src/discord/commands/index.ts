...
export class AutoCompleteHelper {
    static async complete(interaction: AutocompleteInteraction) {
        ...
        const project = interaction.options.getString("project")
            ? parseInt(interaction.options.getString("project"))
            : undefined;
        let choices = [];
        switch (focusedOption.name) {
            ...
            case "card": {
                ...
                const slots =
                    cards.length > 0
                        ? await dataService.slots.read(
                              cards.map((card) => ({ project: card.project, number: card.number }))
                          )
                        : [];
                const releasedSlots = new Set(
                    slots.filter((slot) => slot.release).map((slot) => `${slot.project}|${slot.number}`)
                );
                cards = cards.filter((card) => !releasedSlots.has(`${card.project}|${card.number}`));
                // Reverse to ensure the latest cards are added first
                choices = sortBy(cards, ["project", "number"]).reduce(
                    (chs, card) => {
