...
router.post("/role/:roleId", asyncHandler<{ roleId: string }, MeResponse, unknown, unknown>(async (req, res) => {
    const { realPrincipal: realUser, impersonating } = getClientContext();
    if (impersonating) {
        throw new ApiErrorResponse(StatusCodes.CONFLICT, "Already Impersonating", "Exit your current impersonation before starting a new one");
    }
    ...
    const token = signImpersonationToken({ realDiscordId: realUser.discordId, type: "role", targetId: role.discordId });
    res.cookie(IMPERSONATION_COOKIE, token, impersonationCookieOptions);

    await logActivity(
        LogCategory.AUTH,
        "auth.impersonate.start",
        "<principal> started impersonating role <role>",
        {
            principal: { type: "user", id: realUser.discordId, displayname: realUser.displayname, avatarUrl: realUser.avatarUrl },
...
router.post("/user/:discordId", asyncHandler<{ discordId: string }, MeResponse, unknown, unknown>(async (req, res) => {
    const { realPrincipal: realUser, impersonating } = getClientContext();
    if (impersonating) {
        throw new ApiErrorResponse(StatusCodes.CONFLICT, "Already Impersonating", "Exit your current impersonation before starting a new one");
    }
    ...
    if (discordId === realUser.discordId) {
        throw new ApiErrorResponse(StatusCodes.UNPROCESSABLE_ENTITY, "Invalid Target", "You cannot impersonate yourself");
    }
    ...
    const token = signImpersonationToken({ realDiscordId: realUser.discordId, type: "user", targetId: targetUser.discordId });
    res.cookie(IMPERSONATION_COOKIE, token, impersonationCookieOptions);
...
router.post("/stop", asyncHandler<unknown, MeResponse, unknown, unknown>(async (_req, res) => {
    const { realPrincipal: realUser, impersonating } = getClientContext();
    ...
    if (impersonating) {
        await logActivity(
            ...
            "<principal> stopped impersonating",
            { principal: { type: "user", id: realUser.discordId, displayname: realUser.displayname, avatarUrl: realUser.avatarUrl } }
        );
    }
