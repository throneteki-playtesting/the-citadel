...
const SuggestionsGrid = ({
    ...
    const filter = useFilter<ICardSuggestion>({
        archivedReason: undefined,
        card: { faction: factions, type: types },
        user: { discordId: users.map((user) => user.discordId) },
        tags
    });
    ...
    return (
        <div className={classNames("w-full flex flex-col gap-2", className)} style={style}>
            <div className="flex flex-wrap gap-2">
                {!hideFilters?.faction && (
                    <FactionFilter className="flex-1" factions={factions} setFactions={setFactions} />
                )}
                {!hideFilters?.type && <TypeFilter className="flex-1" types={types} setTypes={setTypes} />}
                {!hideFilters?.user && (
                    <UserFilter className="sm:flex-1" label="Suggested By" users={users} setUsers={setUsers} />
                )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center">
                <div className="w-full flex gap-2">
                    <Select
                        ...
                        onSelectionChange={(keys) => setPerPage(keys.currentKey ? parseInt(keys.currentKey) : 12)}
                    >
                        {[12, 24, 36, 48].map((pp) => (
                    ...
                    <Pagination
                        ...
                        page={page}
                        onChange={setPage}
                        total={Math.ceil((suggestionsData?.total ?? 0) / perPage)}
                    />
                </div>
                <OrderBySelector
                    className="sm:w-lg"
