...
export default function Roles() {
    ...
    const renderCell = useCallback(
        (role: RoleWithUserCount, columnKey: Key) => {
            switch (columnKey) {
                case "name":
                    ...
                    return (
                        <div
                            key={role.discordId}
                            className={classNames("w-fit border py-1 px-2 rounded-md text-sm", {
                                "border-content3": !hex
                            })}
                            style={
                                hex ? { backgroundColor: `${hex}33`, borderColor: `${hex}66`, color: hex } : undefined
                            }
                        >
                            {role.name}
                        </div>
        ...
        <div className="space-y-2">
            <div className="px-4 md:px-0 space-y-2">
                ...
                <div className="text-sm md:text-base">
                    Roles are managed within Discord, but can have citadel permissions tied to them.
                </div>
            </div>
            <Table
                ...
                bottomContent={
                    <div className="px-4">
                        <Pagination
                            page={page}
                            onChange={setPage}
                            total={Math.max(1, Math.ceil((rolesData?.total ?? 0) / perPage))}
                        />
                    </div>
                }
            >
                ...
                <TableBody
                    emptyContent="There are no roles configured on discord server"
