...
export function findNextAvailableIndex(itemIds: string[], slots: ReleaseSlotAllocation[] | undefined, faction: Faction | undefined): number {
    return itemIds.findIndex((id, index) => slotNumberFromItemId(id) === undefined && isFactionCompatible(getPositionFaction(slots, index + 1), faction));
}
    ...
    return {
        ...
        keyframes: ({ dragOverlay, transform, droppableContainers }) => {
            if (isPoolDrop()) {
                ...
                const poolRect = desktopRect && desktopRect.width > 0 && desktopRect.height > 0 ? desktopRect : mobileRect;
                if (poolRect) {
                    // dragOverlay.rect tracks the live position; active.rect is the pre-drag source spot
                    const dx = (poolRect.left + poolRect.width / 2) - (dragOverlay.rect.left + dragOverlay.rect.width / 2);
                    const dy = (poolRect.top + poolRect.height / 2) - (dragOverlay.rect.top + dragOverlay.rect.height / 2);
...
function poolAwareCollision(args: Parameters<CollisionDetection>[0], poolState: { isOpen: boolean, poolItemIds: ReadonlySet<string> }) {
    ...
    if (poolState.isOpen) {
        droppableContainers = droppableContainers.filter((container) =>
            container.id === POOL_ID || container.id === POOL_MOBILE_TRIGGER_ID || poolState.poolItemIds.has(String(container.id)));
    }
...
export const collisionDetection: CollisionDetection = (args) => poolAwareCollision(args, { isOpen: false, poolItemIds: EMPTY_SET });

export function createPoolAwareCollisionDetection(getPoolState: () => { isOpen: boolean, poolItemIds: ReadonlySet<string> }): CollisionDetection {
