...
export function SortableReleaseBlock(props: ReleaseBlockProps) {
    ...
    return (
        <div ref={setNodeRef} style={style} className="relative">
            {props.isReorderDragging && <div className="absolute inset-0 border-2 border-dashed border-content3" />}
...
export function ReleaseBlockOverlay({
    release,
    filledCount,
    canEditReleases,
    canDeleteReleases
}: ReleaseBlockOverlayProps) {
...
const ReleaseBlockHeader = memo(function ReleaseBlockHeader({
    ...
    canEditReleases,
    canDeleteReleases,
    publishTooltip = "Publish Release",
    ...
    return (
        <div
            className="flex items-center gap-2 px-4 py-3 bg-content2 border-b border-content3 cursor-pointer"
        ...
        >
            ...
            <span
                className={classNames(
                    ...
                    isReorderDragging ? "w-0 -mr-2 opacity-0" : "w-2.5 opacity-100"
                )}
            >
                <FontAwesomeIcon
                    icon={faChevronRight}
                    className={classNames("text-foreground/50 transition-transform duration-200", {
            ...
            <div className="flex-1 min-w-0 flex flex-col items-start gap-1">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <TouchTooltip
                        content={<span className="text-sm font-sans">{releaseStatusDescriptions[displayStatus]}</span>}
                        size="sm"
                        delay={0}
                        closeDelay={0}
                    >
            ...
            {(canCopyCodes || (!isLocked && (canEditReleases || canDeleteReleases))) && (
                <div
                    className={classNames(
                        ...
                        isReorderDragging ? "opacity-0 pointer-events-none" : "opacity-100"
                    )}
                    onClick={(e) => e.stopPropagation()}
                >
                    <Dropdown>
                                ...
                                ...(!isLocked && canEditReleases
                                    ? [
                                          ...
                                          <DropdownItem
                                              ...
                                              description={isPublishDisabled ? publishTooltip : undefined}
                                          >
                                              Publish
                                          </DropdownItem>
                                      ]
...
export function ReleaseBlock({
    ...
    const isDirectValidTarget =
        overIndexInRelease !== -1 &&
        isFactionCompatible(getPositionFaction(release.slots, overIndexInRelease + 1), activeFaction);
    const nextAvailableIndex = findNextAvailableIndex(itemIds, release.slots, activeFaction);
    const showNextSlotHighlight =
        isHoveringRelease && !isDirectValidTarget && !isOwnRelease && nextAvailableIndex !== -1;
    ...
    const onDelete = async () => {
        try {
            ...
            addToast({
                ...
                color: "success",
                description: `Release '${release.code}' has been deleted`
            });
        } catch {
                            ...
                            <SortableContext items={itemIds} strategy={noReorderPreview}>
                                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 p-3">
                                    {itemIds.map((id, index) => {
                                        const slotNumber = slotNumberFromItemId(id);
                                        const card =
                                            slotNumber !== undefined ? cardsByNumber.get(slotNumber) : undefined;
                                        return (
                                            <ReleasePositionSlot
                                                key={id}
                                                id={id}
...
export function ReleasePositionSlot({
    id,
    position,
    faction,
    card,
