...
function normalizeSlots(slots?: DeepPartial<ReleaseSlotAllocation[]>): ReleaseSlotAllocation[] {
    const existing = (slots ?? releaseTemplates.chapterPack.slots!).filter(
        (allocation): allocation is ReleaseSlotAllocation => !!allocation?.faction
    );
...
export default function EditReleaseModal({
    ...
    useEffect(() => {
        const slots = isExpansion
            ? (initial?.slots ?? []).filter(
                  (allocation): allocation is ReleaseSlotAllocation =>
                      !!allocation?.faction && (allocation.count ?? 0) > 0
              )
    ...
    return (
        <Modal isOpen={isOpen} placement="center" onOpenChange={(isOpen) => !isOpen && onModalClose?.(false)}>
            <ModalContent>
                {(onClose) => (
                    <Wizard schema={Release.Draft} onSubmit={onSubmit} data={release}>
                                ...
                                <WizardPage controlledData={release}>
                                    ...
                                    <div
                                        className={classNames(
                                            "gap-2 w-full",
                                            isExpansion ? "flex" : "grid grid-cols-2"
                                        )}
                                    >
                                        <Input
                                            name="code"
                                        ...
                                        {!isExpansion && (
                                            ...
                                            >
                                                {Object.entries(releaseTemplates).map(
                                    ...
                                    <Select
                                        ...
                                        onSelectionChange={(keys) =>
                                            setRelease((prev) => ({
                                                ...prev,
                                                status: [...keys][0] as IProjectRelease["status"]
                                            }))
                                        }
                                    >
                                    ...
                                    <DatePicker
                                        ...
                                        onChange={(date) =>
                                            setRelease((prev) => ({
                                                ...prev,
                                                plannedDate: date ? (date.toString() as ReleaseDate) : undefined
                                            }))
                                        }
                                        endContent={
                                            release.plannedDate && (
                                                <Button
                                                    ...
                                                    onPress={() =>
                                                        setRelease((prev) => ({ ...prev, plannedDate: undefined }))
                                                    }
                                                >
                                                    <FontAwesomeIcon
                                                        icon={faXmarkCircle}
                                ...
                                {(isExpansion || template === "custom") && (
                                    <WizardPage controlledData={{ slots: release.slots }}>
                                        <FactionSlotsEditor
                                            slots={
                                                isExpansion
                                                    ? ((release.slots ?? []) as ReleaseSlotAllocation[])
                                                    : normalizeSlots(release.slots)
                                            }
...
function FactionSlotsEditor({ slots, lockCounts = false, onChange }: FactionSlotsEditorProps) {
    ...
    const onCountChange = (faction: ReleaseSlotAllocation["faction"], count: number) => {
        onChange(
            slots.map((allocation) =>
                allocation.faction === faction ? { ...allocation, count: isNaN(count) ? 0 : count } : allocation
            )
        );
    };
    ...
    return (
        <div className="w-full flex flex-col gap-2">
            ...
            >
                <SortableContext
                    items={slots.map((allocation) => allocation.faction)}
...
function FactionSlotRow({ allocation, lockCount, onCountChange }: FactionSlotRowProps) {
    ...
    return (
        ...
        >
            ...
            <span className="flex-1 min-w-0 truncate font-cinzel tracking-wide">
                {factionNames[allocation.faction]}
            </span>
            <NumberInput
