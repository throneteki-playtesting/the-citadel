...
import { useAssignSlotReleaseMutation, useGetCardsQuery, useGetSlotsQuery, useReorderReleasesMutation } from "../../../api";
import api from "../../../api";
...
export default function ProjectReleases({ project }: ProjectReleasesProps) {
    ...
    const commitMove = async (slotNumber: number, code: string | null, position?: number) => {
        const payload = code === null
            ? { project: project.number, number: slotNumber, code: null as null }
            : { project: project.number, number: slotNumber, code, position: position! };
        ...
        const patchResult = dispatch(
            api.util.updateQueryData("getSlots", { project: project.number }, (draft) => {
