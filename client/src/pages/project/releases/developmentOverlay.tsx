...
export default function DevelopmentOverlay({
    ...
    const { setNodeRef: setBubbleDropRef, isOver: isBubbleOver } = useDroppable({
        id: POOL_MOBILE_TRIGGER_ID,
        disabled: isOpen
    });
    ...
    return createPortal(
        <>
            <AnimatePresence>
                {!isOpen && (
                    <motion.button
                        ...
                        disabled={count === 0}
                        onClick={() =>
                            count > 0 && bubbleRef.current && onOpen(bubbleRef.current.getBoundingClientRect())
                        }
                    ...
                    >
                        <div
                            className={classNames(
                                ...
                                count === 0
                                    ? "border-content3 bg-content1 opacity-40"
                                    : isBubbleOver
            ...
            <motion.div
                ...
                className={classNames("fixed z-30 border-2 border-content3 bg-content1 shadow-2xl flex flex-col", {
...
export function DevelopmentPoolButton({ count, isOpen, onOpen }: DevelopmentPoolButtonProps) {
    ...
    return (
        <div
            ref={combinedRef}
            className={classNames("hidden sm:block relative p-2 -m-2", { "pointer-events-none": isOpen })}
        >
            <Button
                size="sm"
                variant="flat"
                isDisabled={isOpen || count === 0}
                className={classNames("transition-all", isOver ? "bg-primary/20 text-primary scale-110" : undefined, {
                    "opacity-40": isOpen
                })}
