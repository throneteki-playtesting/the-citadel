...
const ProjectHeaderPlaytestingUpdateNotice = ({
    className,
    style,
    project
}: ProjectHeaderPlaytestingUpdateNoticeProps) => {
    ...
    return (
        <>
            {!isDraftsLoading && draftCards.length > 0 && (
                <Alert
                    color="primary"
                    title="Playtesting Updates Available"
                    classNames={{ title: "font-bold text-sm md:text-md lg:text-lg" }}
                    className={className}
                    style={style}
                >
                    <div className="text-xs md:text-sm lg:text-md italic">
                        {draftCards.length} draft card changes found. To push these changes to playtesters, a
                        Playtesting Update must be created & submitted.
                    </div>
                    <Button
                        variant="flat"
                        className="mt-2 w-full md:text-lg font-bold"
                        onPress={() => setIsModalOpen(true)}
                    >
                        Create Playtesting Update
                    </Button>
                </Alert>
            )}
