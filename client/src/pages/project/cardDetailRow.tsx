...
import { parseCardCode, thronesColors, titleCase, typeNames } from "common/utils";
...
export interface ICardDetailRow {
    card: IPlaytestCard;
    illustrator?: string;
    portfolio?: string;
    ...
    packCode?: string;
    /** The viewer's own release-check verdict for this slot, where one has been submitted */
    releaseCheckEntry?: IReleaseCheck;
    /** Whether the release-check button is worth showing at all - mirrors the release board's own gate */
    wantsReleaseCheck: boolean;
}
...
type CardDetailRowProps = {
    entry: ICardDetailRow;
    onOpenReleaseCheck: (card: IPlaytestCard) => void;
};

// Field-by-field so a card whose own data hasn't changed skips re-rendering its (tooltip-heavy)
// row when a filter change only adds/removes other cards around it in the list.
function areRowPropsEqual(prev: CardDetailRowProps, next: CardDetailRowProps): boolean {
    return (
        prev.onOpenReleaseCheck === next.onOpenReleaseCheck &&
        prev.entry.card === next.entry.card &&
        prev.entry.illustrator === next.entry.illustrator &&
        prev.entry.portfolio === next.entry.portfolio &&
        prev.entry.commissionNote === next.entry.commissionNote &&
        prev.entry.releaseNumber === next.entry.releaseNumber &&
        prev.entry.packCode === next.entry.packCode &&
        prev.entry.releaseCheckEntry === next.entry.releaseCheckEntry &&
        prev.entry.wantsReleaseCheck === next.entry.wantsReleaseCheck
    );
}

// Raw card data, styled after the Artworks row (accent stripe). Desktop splits stats left / text right;
// mobile folds to one vertical stack, stats above text.
const CardDetailRow = memo(function CardDetailRow({ entry, onOpenReleaseCheck }: CardDetailRowProps) {
    const { card } = entry;
            ...
            <div className="flex-1 min-w-0 py-2 pr-2 sm:pr-3 flex flex-col gap-2">
                <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {card.released && (
                        ...
                        >
                            <Chip
                                ...
                                variant="flat"
                                color="success"
                                startContent={<FontAwesomeIcon icon={faCheck} className="pl-1.5 text-[0.6rem]" />}
                                classNames={{ base: "h-5 cursor-help", content: "px-1 text-[0.65rem]" }}
                            >
                                Released
                            </Chip>
                        </TouchTooltip>
                    ...
                    <div className="ml-auto flex items-center gap-1">
                        {entry.wantsReleaseCheck && (
                            <ReleaseCheckButton
                                entry={entry.releaseCheckEntry}
                                latestVersion={card.version}
                                onPress={() => onOpenReleaseCheck(card)}
                            />
                        )}
                        <DraftActions project={card.project} number={card.number} showDivider={false} />
                    </div>
                </div>
