...
type Submission =
    ...
    | ({ key: string; type: "review" } & IPlaytestReview);
...
export default function RecentSubmissions() {
    ...
    const { data: reviewData, isLoading: isReviewDataLoading } = useGetReviewsQuery(
        { orderBy: { updated: "desc" }, page: 1, perPage: 5 },
        { skip: !hasReviewReadPermission }
    );
    const { data: suggestionData, isLoading: isSuggestionDataLoading } = useGetSuggestionsQuery(
        { orderBy: { updated: "desc" }, page: 1, perPage: 5 },
        { skip: !hasSuggestionReadPermission }
    );

    const isLoading = useMemo(
        () => isReviewDataLoading || isSuggestionDataLoading,
        [isReviewDataLoading, isSuggestionDataLoading]
    );
    ...
    const submissions = useMemo(() => {
        ...
        return [
            ...reviews.map(
                (r) =>
                    ({ key: `${r.project}|${r.number}|${r.version}|${r.reviewer}`, type: "review", ...r }) as Submission
            ),
    ...
    const content = useMemo(() => {
        ...
        return submissions?.map((submission) => <SubmissionRow key={submission.key} submission={submission} />);
...
function ReviewRow({ review }: ReviewRowProps) {
    ...
    return (
        ...
        >
            <PermissionedLink
                to={`/project/${review.project}/${review.number}`}
                state={{ highlight: highlightTarget.review(review) }}
                requires={Permission.READ_REVIEWS}
            >
                <div className="relative min-w-0 space-y-0.5 px-4 py-2">
                    ...
                    <div className="flex gap-2 items-center">
                        <Avatar
                            ...
                            className="shrink-0 size-10 sm:size-8 md:size-10"
                        />
                        <div className="flex flex-col gap-1 min-w-0">
...
function StatementBars({ statements }: { statements: Statements }) {
    ...
    return (
        <div className="flex m-0 gap-1">
            {values.map((score, i) => (
                <div key={i} className={classNames("h-1 w-5 bg-disagree", scoreBarClass[score])} />
            ))}
        </div>
    );
}
