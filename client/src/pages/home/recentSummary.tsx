...
const gridClasses = ["", "grid-cols-1", "grid-cols-1 sm:grid-cols-2", "grid-cols-1 sm:grid-cols-3", "grid-cols-1 sm:grid-cols-2"] as const;
...
export default function RecentSummary({ children }: RecentSummaryProps) {
    ...
    return (
        <div className={classNames("grid gap-2", gridClasses[visibleCount])}>
            {children}
        </div>
    );
}
