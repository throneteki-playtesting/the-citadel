...
const OrderBySelector = function <T>({
    label = "Order By",
    orderBy = {},
    setOrderBy,
    options,
                    ...
                    <div className="py-1 flex flex-wrap gap-1">
                        {items.map((item) => (
                            <Chip
                                ...
                                color="default"
                                radius="sm"
                                endContent={<FontAwesomeIcon icon={getIcon(internal[item.data!.key])} />}
                            >
                                {item.data!.value}
                            </Chip>
                        ))}
...
type OrderByProps<T> = {
    label?: string;
    orderBy?: Sort<T>;
    setOrderBy: (orderBys?: Sort<T>) => void;
    options: EntriesOf<T, string>;
