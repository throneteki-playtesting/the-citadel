    ...
    const handleSelectionChange = useCallback(
        (keys: SharedSelection) => {
            if (keys === "all") {
                ...
                setUsers(items.filter((user) => Array.from(keys).includes(user.discordId)));
            }
        },
        [items, setUsers]
    );
    ...
    return (
        <SearchableMultiSelect
            ...
            renderSelected={(selected) =>
                selected.map((user) => <Avatar key={user.discordId} size="sm" src={user.avatarUrl} />)
            }
            renderItem={(user) => (
...
type UserFilterProps = Omit<BaseElementProps, "children"> & {
    label?: string;
