...
export default function PlaytestingUpdateMiniCard({
    ...
    detailed = false,
    pinched = false
}: PlaytestingUpdateMiniCardProps) {
    const { data: project, isLoading: isProjectLoading } = useGetProjectQuery({ number: playtestingUpdate.project });
    const { data: cards, isLoading: isCardsLoading } = useGetPlaytestingUpdateCardsQuery({
    ...
    return (
        <div className="h-full">
            <PermissionedLink
                ...
                to={`/project/${playtestingUpdate.project}/update/${playtestingUpdate.version}`}
                requires={Permission.READ_PLAYTESTING_UPDATES}
            >
                <div
                    className={classNames("p-4 space-y-1 hover:bg-content2 transition-colors", className)}
                    style={style}
                >
                                ...
                                .map(([type, count]) => (
                                    <div
                                        ...
                                        className={classNames(
                                            "bg-content3/50 px-2 text-xs rounded-full font-sans opacity-50 border-1",
                            ...
                            {Object.entries(factionCounts)
                                ...
                                .map(([faction, count]) => (
                                    <TouchTooltip
                                        key={faction}
                                        content={
                                            <div className="text-sm leading-0 font-cinzel">
                                                <ThronesIcon name={faction as Faction} /> {count} Change
                                                {count > 1 ? "s" : ""}
                                            </div>
                                        }
                                        size="sm"
                                    ...
                                    >
                                        <div
                                            className={classNames(
                                                factionBarClasses[faction as Faction],
    ...
    const { reviewed, total, played } = useMemo(() => {
        ...
        for (const [number, version] of changes) {
            const cardReviews = reviews.filter(
                (review) => review.number === Number(number) && review.version === version
            );
    ...
    return (
        <div className="pt-1 space-y-1">
            <div className="flex items-baseline justify-between gap-2">
                ...
                <div className="text-xs font-sans text-foreground/60">
                    {reviewed}/{total} change{total !== 1 ? "s" : ""} playtested
                </div>
            </div>
            ...
            <div className="text-xs font-serif italic text-foreground/40">
                {played} game{played !== 1 ? "s" : ""} played across these changes
            </div>
        </div>
    );
}
