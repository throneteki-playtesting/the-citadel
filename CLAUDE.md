...
- `components/slidingPages.tsx` — the Wizard's slide-and-measure behaviour extracted for reuse. `WizardPages` is now a thin wrapper over it, and the project's Artworks tab uses it to travel to the artwork editor rather than opening a dialog. Each page is wrapped in a **`PageActiveContext`** (`hooks/useIsPageActive`) saying whether it is the one on show. `inert` and clipping only reach what a page draws inside its own subtree, so anything it portals elsewhere has to be told to stand down — see `ProcessActions`. It defaults to `true`, so nothing outside a `SlidingPages` is affected. Clipping is **`overflow-clip`, not `overflow-hidden`**: hidden is still a scroll container, so focusing a field on the page which has slid in lets the browser scroll the box and drag the page beside it back into view — on a phone that surfaced the Artworks filters along the screen edge. Inactive pages are clipped to their own width as well, so a chip row's negative margin can't overhang past the active page; the active page stays unclipped so its tooltips and popovers still escape. The measured height eases via Tailwind's **`transition-height`**, but only from the second measurement onward — a `hasMeasuredOnce` flag (set one render behind `measuredHeight`, on purpose) withholds the class for the very first, unset-to-real change. That first change is "auto" (collapsed, since the children are absolutely positioned) easing to a pixel value, which the browser can't interpolate and instead renders as the box sitting collapsed until the transition resolves — under a React Router `startTransition` navigation, that resolution lands late enough to paint as a real, visible blank box before content snaps in. Dropping the transition outright was tried instead, to dodge that case — but it also killed the animation between two already-measured pages (eg. Wizard Back/Next), which is a real value easing to another real value and animates cleanly. Splitting the two cases is what keeps both working.
